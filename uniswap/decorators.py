...
from .constants import BNB_ADDRESS, ETH_ADDRESS
...
def check_approval(method: Callable) -> Callable:
    ...
    def approved(self: Any, *args: Any, **kwargs: Any) -> Any:
        ...
        token = args[0] if args[0] != ETH_ADDRESS and args[0] != BNB_ADDRESS else None
        token_two = None

        # Check second token, if needed
        if method.__name__ == "make_trade" or method.__name__ == "make_trade_output":
            token_two = args[1] if args[1] != ETH_ADDRESS and args[1] != BNB_ADDRESS else None
